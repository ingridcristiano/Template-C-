...
int ucas::getOtsuAutoThreshold(const std::vector<int> & data) 
{
	...
	int k_x10,kStar;  // k = the current threshold; kStar = optimal threshold
	...
	// Initialize values:
	S = N = 0;
	for (k_x10=0; k_x10<L; k_x10++){
		S += k_x10 * data[k_x10];	// Total histogram intensity
		N += data[k_x10];		// Total number of data points
	}
	...
	for (k_x10=1; k_x10<L-1; k_x10++) { // No need to check endpoints k = 0 or k = L-1
		Sk += k_x10 * data[k_x10];
		N1 += data[k_x10];
