		...
		try
		{
			...
			while(job >= 0 && !error)
			{
				...
				if(job >= 0)
				{
					...
					int k_x10 = nPoints-job-1;
					double tpr = 0;
					double tpr_sq = 0;
					for(size_t i=0; i<rocs.size(); i++)
					{
						double tpr_int = ROC_interp_tpr<double>(rocs[i], avg_roc[k_x10].fpr);
						tpr += tpr_int;
						tpr_sq += tpr_int*tpr_int;

						avg_roc[k_x10].P += rocs[i][0].P;
						avg_roc[k_x10].N += rocs[i][0].N;
					}

					// store result (mutual exclusion guaranteed by 'job')
					avg_roc[k_x10].tpr = tpr/rocs.size();
					double sqrt_arg = tpr_sq/rocs.size() - std::pow(tpr/rocs.size(), 2.0f);
					if(sqrt_arg > 0)
						avg_roc[k_x10].tpr_std = std::sqrt(sqrt_arg);
					else
						avg_roc[k_x10].tpr_std = 0;
					avg_roc[k_x10].P = ucas::round(avg_roc[k_x10].P / double(rocs.size()));
					avg_roc[k_x10].N = ucas::round(avg_roc[k_x10].N / double(rocs.size()));
				}
			}
		}
		catch (ucas::Error & e)
	...
	
	{
		...
		for(int s=0; s<2; s++)
		{
			...
			if(!append_sco || (append_sco && !filexists))
				f << "#SAMPLE \t#SCORE\n";
			for(size_t k_x10=0; k_x10 < (s ? pos_scores.size() : neg_scores.size()); k_x10++)
				f << ucas::strprintf("%8d\t%f\n", k_x10, s ? pos_scores[k_x10] : neg_scores[k_x10]);
			f.close();
		}
	}
